...
module.exports = HandlebarsCompiler.template({"1":function(depth0,helpers,partials,data) {
  ...
  return "        <tr class=\"checkpoints\">\n            <td class=\"action delete\" actionUrl=\""
    + alias2(alias1((depth0 != null ? depth0.deleteUrl : depth0), depth0))
    + "\"><span class=\"yui-button yui-push-button danger\"><button type=\"button\">Delete</button></span></td>\n            <td class=\"action restart\" actionUrl=\""
    + alias2(alias1((depth0 != null ? depth0.restartUrl : depth0), depth0))
    + "\"><span class=\"yui-button yui-push-button primary\"><button type=\"button\">Restart</button></span></td>\n            <td class=\"name\">"
    + alias2(alias1((depth0 != null ? depth0.name : depth0), depth0))
    + "</td>\n        </tr>\n";
},"compiler":[6,">= 2.0.0-beta.1"],"main":function(depth0,helpers,partials,data) {
    var stack1;

  return "<div class=\"caption alert\">\n    This Pipeline run can be restarted from the following Checkpoint(s)\n</div>\n<table>\n"
    + ((stack1 = helpers.each.call(depth0,depth0,{"name":"each","hash":{},"fn":this.program(1, data, 0),"inverse":this.noop,"data":data})) != null ? stack1 : "")
    + "</table>";
},"useData":true});

},{"jenkins-handlebars-rt/runtimes/handlebars3_rt":5}],11:[function(require,module,exports){
require('jenkins-js-modules').whoami('undefined:stageview');

require('jenkins-js-modules')
    .import('jquery-detached:jquery2', 'handlebars:handlebars3', 'pipeline-stage-view:stageview')
...
// The actual stage view ExtensionPoint API instance (from the OSS pipeline stage view) will
// be imported here at runtime. See the withExternalModuleMapping instructions in gulpfile.js.
var stageView = require('jenkins-js-modules').require('pipeline-stage-view:stageview');
var ExtensionPointContribution = stageView.ExtensionPointContribution;
var $ = require('jenkins-js-modules').require('jquery-detached:jquery2').getJQuery();

var jobUrl = $("div[cbwf-controller='pipeline-staged']").attr('objecturl');
var checkpointInfoUrl = jobUrl + 'wfapi-checkpoints';
var checkpointDialogTemplate = require('./checkpoint-restart.hbs');

function enrichExtensionPoints() {
    var runIds = getRunIds();
    if (runIds.length > 0) {
        // Get the checkpoint information for the runs that are currently displayed.
        $.ajax({
            url: checkpointInfoUrl + '?' + runIds, 
            type: 'get', dataType: 'json',
            cache: false, // Force caching off for IE (and anything else)
            success: function (checkpointInfo) {
                // Iterate over the ExtensionPoint definitions and enrich
                // as appropriate...
                stageView.extensionPointContainer.forEach(function(extensionPoint) {
                    if (extensionPoint.type === 'jenkins.pipeline.run') {
                        var runId = extensionPoint.id;
                        var runCheckpointInfo = getRunCheckpoints(checkpointInfo, runId);
                        if (runCheckpointInfo && runCheckpointInfo.length > 0) {
                            // Contribute checkpoint control to the run. 
                            contributeCheckpoints(extensionPoint, runCheckpointInfo);
                        }
                    } else if (extensionPoint.type === 'jenkins.pipeline.run.stage') {
                        var idTokens = extensionPoint.id.split(':');
                        var stageRunId = idTokens[0];
                        var stageId = idTokens[1];
                        var stageCheckpointInfo = getStageCheckpoints(checkpointInfo, stageRunId, stageId);
                        if (stageCheckpointInfo && stageCheckpointInfo.length > 0) {
                            // Contribute checkpoint control to the stage.                             
                            contributeCheckpoints(extensionPoint, stageCheckpointInfo);
                        }
                    }
                });
            }
...
function contributeCheckpoints(extensionPoint, checkpointInfo) {
    var theContribution = new ExtensionPointContribution()
        .setActivator('glyphicon glyphicon-repeat')
        .setCaption('Resume')
        .setContent('<div class="cbwf-checkpoint-restart"></div>')
        .onShow(function () {
            var container = $('.cbwf-checkpoint-restart', $(this));
            container.append(checkpointDialogTemplate(checkpointInfo));

            $('.action .yui-button', container).click(function () {
                var clickedBtn = $(this);
                var actionAjaxUrl = clickedBtn.closest('.action').attr('actionUrl');
                
                new Ajax.Request(actionAjaxUrl, {
                    contentType: "application/json",
                    method: 'post',
                    onSuccess: function() {
                        theContribution.hide();
                    }
                });
            });
        });
    extensionPoint.contribute(theContribution);
}

// Listen for ExtensionPoint definition changes.
stageView.extensionPointContainer.onchange(function() {
    enrichExtensionPoints();
});
...
function getRunIds() {
    ...
    stageView.extensionPointContainer.forEach(function (extensionPoint) {
        if (extensionPoint.type === 'jenkins.pipeline.run') {
            ...
            runIds += ('runId=' + extensionPoint.id);
        }
    });
    return runIds;
}

function getRunCheckpoints(checkpointInfo, runId) {
    return checkpointInfo[runId];
}

function getStageCheckpoints(checkpointInfo, runId, stageId) {
    var runCheckpoints = getRunCheckpoints(checkpointInfo, runId);
    var stageCheckpoints = [];    
    for (var i = 0; i < runCheckpoints.length; i++) {
        var runCheckpoint = runCheckpoints[i];
        if (runCheckpoint.stageId === stageId) {
            stageCheckpoints.push(runCheckpoint);
        }
    }
...
enrichExtensionPoints();
		...
		require('jenkins-js-modules').export(undefined, 'stageview', {});
    });


},{"./checkpoint-restart.hbs":10,"jenkins-js-modules":6}]},{},[11]);
